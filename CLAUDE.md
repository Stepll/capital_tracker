...
backend/
  ...
  src/
    ...
    CapitalTracker.Application/    CQRS use-case'и (MediatR), DTO
      ...
      Transfer/      Формат CSV (PortfolioCsv), читач (CsvReader/PortfolioCsvParser),
                     експорт та імпорт трьох скоупів, партії й скасування
...
- **Кодування поки тільки UTF-8** — файл в іншому кодуванні відхиляється з поясненням, а
  не читається в кракозябри. `windows-1251` прийде разом із мапером колонок
...
15. **Наступне:** мапер колонок для чужих виписок (+ `windows-1251`) і пресети під
   брокерів;
