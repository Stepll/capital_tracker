...
- **Напрямок операції в кожному файлі живе в іншому місці**, тому їх три: значення в
  колонці (`кредит`/`дебет`, `BUY`/`SELL`), знак суми (карткова виписка не має жодної
  колонки типу), або одна подія на весь файл. У режимі знаку рядок **без цифр
  пропускається** — інакше підвал банку («Вклади гарантуються…») став би внеском на нуль
...
16. **Наступне:** режим «баланс» для банківських виписок (остання строка → `Оцінка`,
   без імпорту сотні платежів), пресети під брокерів і `windows-1251`;
