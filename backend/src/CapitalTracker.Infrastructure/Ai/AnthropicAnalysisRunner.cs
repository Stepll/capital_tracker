...
/// <summary>
/// The Anthropic call itself, shared by every analysis scope: request shape, streaming,
/// tool-input accumulation, refusal handling and result parsing. Only the system prompt
/// and the user turn differ between a per-asset and a portfolio analysis, so only those
/// are parameters — everything below is where the SDK's sharp edges live, and they are
/// not worth learning twice.
/// </summary>
public class AnthropicAnalysisRunner(
    AnthropicClient client,
    IOptions<AnthropicOptions> options,
    ILogger<AnthropicAnalysisRunner> logger)
{
    private const int DetailMaxLength = 140;

    /// <param name="subject">Only used in logs — what the run was about.</param>
    public async IAsyncEnumerable<AnalysisGenerationEvent> RunAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        {
            logger.LogWarning("Anthropic declined to analyse {Subject}.", subject);
        ...
        if (result is null)
        {
            logger.LogWarning("Analysis of {Subject} ended without a save_analysis call (stop reason: {StopReason}).",
                subject, stopReason ?? "none");
    ...
    private AnalysisResult? ParseResult(string json)
    {
        try
        {
            ...
            return new AnalysisResult(summary!, facts);
        }
    ...
    public MessageCreateParams BuildParameters(string systemPrompt, string userMessage) => new()
    {
